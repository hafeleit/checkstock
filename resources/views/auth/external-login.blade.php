    ...
    <div class="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
        <div class="w-full max-w-md space-y-8">
            ...
            <div class="flex bg-gray-100 rounded-md p-1">
                <button type="button" data-tab="employee" class="login-tab flex-1 text-sm font-medium py-2 rounded-md transition {{ $activeLoginTab === 'employee' ? 'bg-white shadow text-gray-900' : 'text-gray-500' }}">
                    Employee
                </button>
                <button type="button" data-tab="external" class="login-tab flex-1 text-sm font-medium py-2 rounded-md transition {{ $activeLoginTab === 'external' ? 'bg-white shadow text-gray-900' : 'text-gray-500' }}">
                    External
                </button>
            </div>

            <!-- Employee panel: Azure AD SSO -->
            <div id="employee-panel" class="login-panel space-y-4 {{ $activeLoginTab === 'employee' ? '' : 'hidden' }}">
                <a href="{{ route('microsoft.redirect') }}" class="w-full h-11 flex items-center justify-center gap-2 px-4 py-2 border border-gray-300 rounded-md shadow-sm text-gray-700 font-medium hover:bg-gray-50 transition">
                ...
                <p class="text-xs text-gray-500 text-center">Employees sign in using their company Microsoft account.</p>
            </div>

            <!-- External panel: email/password login -->
            <div id="external-panel" class="login-panel {{ $activeLoginTab === 'external' ? '' : 'hidden' }}">
                <!-- Login Form -->
        ...
        loginTabs.forEach(function(tab) {
            tab.addEventListener('click', function() {
                ...
                Object.keys(loginPanels).forEach(function(key) {
                    loginPanels[key].classList.toggle('hidden', key !== tab.dataset.tab);
                });
            });
        });
