    ...
    <div class="container-fluid py-4">
                ...
                <table class="table table-hover align-items-center mb-0">
                    ...
                    <tbody>
                        @forelse ($commissions as $c)
                            <tr>
                                <td>{{ $c->sub_id }}</td>
                                <td>{{ \Carbon\Carbon::createFromFormat('Ym', substr($c->sub_id, 0, 6))->format('F Y') }}
                                </td>
                                ...
                                <td>
                                    ...
                                    @if (in_array($c->status, ['Summary Confirmed', 'Summary Approved', 'Final Approved']))
                                        @can('Commissions Check')
                                            <a href="#" class="btn btn-sm btn-success commissions-link"
                                                data-id="{{ $c->id }}">
                                    ...
                                    @can('Commissions Delete')
                                        @if (in_array($c->status, ['imported', 'calculated', 'Summary Rejected']))
                                            <form method="POST" action="{{ route('commissions.destroy', $c->id) }}"
        ...
        <div class="modal fade" id="importModal" tabindex="-1" aria-labelledby="importModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <form id="importForm" action="{{ route('commissions.import') }}" method="POST"
                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            ...
                            <input type="file" class="form-control" id="file1" name="file1" accept=".xlsx, .xls">
                        </div>
                    </div>
    ...
    <script nonce="{{ request()->attributes->get('csp_script_nonce') }}">
        document.addEventListener('click', function(e) {
            if (e.target.closest('.commissions-link')) {
                ...
                const commissionId = button.dataset.id;
