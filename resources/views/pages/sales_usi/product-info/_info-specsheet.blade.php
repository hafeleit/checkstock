<div class="card border p-4 mt-3">
    <div class="d-flex align-items-center justify-between">
        ...
        @can('salesusi import specsheets')
        <button type="button" class="btn btn-sm btn-outline-dark d-flex align-items-center gap-2 m-0" data-bs-toggle="modal" data-bs-target="#changeSpecSheetModal">
    ...
    <div class="table-responsive mt-3">
        <table class="table table-hover">
            ...
            <tbody>
                @if (!$specSheets->isEmpty())
                    @foreach ($specSheets as $specSheet)
                        <tr>
                            <td>
                                <a href="{{ $specSheet->path }}" target="_blank" class="d-flex align-items-center gap-2">
...
<script nonce="{{ request()->attributes->get('csp_script_nonce') }}">
    ...
    document.addEventListener('click', function(e) {
        const deleteBtn = e.target.closest('.delete-specsheet-btn');
                ...
                if (result.isConfirmed) {
                    axios.delete(`/product-infos/pdf-files/${fileId}`)
                        .then(response => {
                            Swal.fire(
                                'Deleted!',
                                'Spec Sheet file has been deleted.',
                                'success'
                            ).then(() => {
                                window.location.reload();
                            });
