            ...
            <tbody>
                @if (!$catalogues->isEmpty())
                    @foreach ($catalogues as $catalogue)
                        <tr>
                            <td>
                                <a href="{{ $catalogue->path }}" target="_blank" class="d-flex align-items-center gap-2">
...
<script nonce="{{ request()->attributes->get('csp_script_nonce') }}">
    ...
    document.addEventListener('click', function(e) {
        const deleteBtn = e.target.closest('.delete-catalogue-btn');
                ...
                if (result.isConfirmed) {
                    axios.delete(`/product-infos/pdf-files/${fileId}`)
                        .then(response => {
                            Swal.fire(
                                'Deleted!',
                                'Catalogue file has been deleted.',
                                'success'
                            ).then(() => {
                                window.location.reload();
                            });
