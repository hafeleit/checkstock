    ...
    <div class="mx-auto">
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex justify-between items-center mb-4">
                <div>
                    <h1 class="text-md md:text-xl font-bold text-gray-800">Products 360°</h1>
                    @php
                        $yesterday = date('d/m/Y', strtotime('-1 day'));
                    @endphp
                    <p class="text-xs font-semibold text-gray-400">LAST UPDATE: {{ $yesterday }} 20:00</p>
                </div>
                <div>
                    <a href="#" id="close-button"
                        class="px-4 py-3 bg-gray-500 hover:bg-gray-700 text-white rounded-md transition duration-200">
                        CLOSE
                    </a>
                </div>
            </div>
                            ...
                            <div class="img-thumbnail mb-3">
                                <div class="text-center flex flex-col justify-center items-center h-full p-4">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48"
                                        fill="currentColor" class="bi bi-image text-gray-500" viewBox="0 0 16 16">
                ...
                <div class="md:col-span-4">
                    ...
                    <div class="space-y-2 bg-gray-100 p-4 rounded-xl">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            ])>{{ $productDetail ? $productDetail->item_status : 'N/A' }}</p>
                        </div>
                        <div class="flex items-center gap-2">
                            <label class="block text-sm font-semibold text-gray-700 tracking-wider">MRP: </label>
                            <p @class([
                                'inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-semibold mt-1 bg-green-600 text-white' =>
                                    $productDetail->mrp,
                                'text-md text-gray-500' => empty($productDetail->mrp),
                            ])>
                                {{ $productDetail && $productDetail->mrp ? $productDetail->mrp : 'N/A' }}</p>
                        </div>
                        <div class="flex items-center gap-2">
                            <label class="block text-sm font-semibold text-gray-700 tracking-wider">Storage Indicator:
                            </label>
                            <p @class([
                                'inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-semibold mt-1 bg-green-600 text-white' =>
                                    $productDetail->inventory_code,
                                'text-md text-gray-500' => empty($productDetail->inventory_code),
                            ])>
                                {{ $productDetail && $productDetail->inventory_code ? $productDetail->inventory_code : 'N/A' }}
                            </p>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        {{-- Spec sheets --}}
                        <div class="mt-4">
                            <h2 class="text-lg font-bold text-gray-700 mb-2">Spec Sheets</h2>
                            <div class="overflow-x-auto">
                                <table class="w-full text-left border-collapse bg-white">
                                    @if ($specsheetFiles && !$specsheetFiles->isEmpty())
                                        ...
                                        <tbody class="">
                                            @foreach ($specsheetFiles as $specsheet)
                                                <tr class="hover:bg-gray-100 transition-colors">
                                                    <td class="px-2 py-1">
                                                        <a href="{{ $specsheet->path }}" target="_blank" id="downloadSpecSheet" class="download-link flex items-center gap-2 underline text-gray-600 hover:text-red-500 transition-colors">
                                                            <svg xmlns="http://www.w3.org/2000/svg" width="16"
                                                                height="16" fill="currentColor" class="bi bi-filetype-pdf"
                                                                viewBox="0 0 16 16">
                                                                <path fill-rule="evenodd"
                                                            ...
                                                            <u class="truncate text-sm">{{ $specsheet->file_name }}</u>
                                                        </a>
                                                    </td>
                                                    <td class="px-2 py-1 text-gray-600 whitespace-nowrap text-center">{{ $specsheet->created_at ? $specsheet->created_at->format('d-m-Y') : 'N/A' }}</td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                    @else
                                        <tr>
                                            <td colspan="2" class="text-gray-500 text-sm">No spec sheets found.</td>
                                        </tr>
                                    @endif
                                </table>
                            </div>
