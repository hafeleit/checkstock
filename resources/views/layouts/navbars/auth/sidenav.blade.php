<aside class="sidenav bg-white navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-4 " id="sidenav-main">
  ...
  <div class="collapse navbar-collapse h-auto w-auto " id="sidenav-collapse-main">
    <ul class="navbar-nav">
      ...
      <li class="nav-item">
        <a class="nav-link {{ in_array(Request::segment(1), ['sales-usi', 'product-infos']) ? 'active' : '' }}" data-bs-toggle="collapse" aria-expanded="{{ in_array(Request::segment(1), ['sales-usi', 'product-infos']) ? 'true' : 'false' }}" href="#salesUsiExample">
          <div class="icon icon-shape icon-sm border-radius-md text-center me-2 d-flex align-items-center justify-content-center">
          ...
          <ul class="nav nav-sm flex-column">
            ...
            <li class="nav-item">
              <a class="nav-link {{ Route::currentRouteName() == 'sales-usi.index' ? 'active' : '' }}" href="{{ route('sales-usi.index') }}">
                <span class="sidenav-mini-icon text-xs"> P </span>
                <span class="sidenav-normal"> Products 360° </span>
              </a>
            </li>
            @endcan
            ...
            <li class="nav-item">
              <a class="nav-link {{ Route::currentRouteName() == 'sales-usi.pc' ? 'active' : '' }}" href="{{ route('sales-usi.pc') }}">
                <span class="sidenav-mini-icon text-xs"> P </span>
                <span class="sidenav-normal"> Products 360° - PC </span>
              </a>
            </li>
            @endcan
      ...
      <li class="nav-item ">
        <a class="nav-link {{ Request::segment(1) == 'audit-logs' ? 'active' : '' }}" data-bs-toggle="collapse" aria-expanded="{{ Request::segment(1) == 'audit-logs' ? 'true' : 'false' }}" href="#auditLogsDropdown">
          <div class="icon icon-shape icon-sm border-radius-md text-center me-2 d-flex align-items-center justify-content-center p-0">
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="{{ Request::segment(1) == 'audit-logs' ? 'text-primary' : 'text-dark' }} text-sm opacity-10">
      ...
      <li class="nav-item">
        <a href="{{ route('after-sales.user-dashboard') }}" class="nav-link">
          ...
          <span class="nav-link-text ms-1">After Sales - User Dashboard</span>
        </a>
      </li>
      @endcan
      ...
      @can('warranty view')
      <li class="nav-item">
        <a class="nav-link {{ in_array(Route::currentRouteName(), ['warranty.check','warranty.list','warranty.edit','warranty.log']) ? '' : 'collapsed' }}" data-bs-toggle="collapse" aria-expanded="{{ in_array(Route::currentRouteName(), ['warranty.check','warranty.list','warranty.edit','warranty.log']) ? 'true' : 'false' }}" href="#warranty-menu">
          <div class="icon icon-shape icon-sm border-radius-md text-center me-2 d-flex align-items-center justify-content-center p-0">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="{{ in_array(Route::currentRouteName(), ['warranty.check','warranty.list','warranty.edit','warranty.log']) ? 'text-primary' : 'text-dark' }}" width="20" height="20">
              <path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75 11.25 15 15 9.75m-3-7.036A11.959 11.959 0 0 1 3.598 6 11.99 11.99 0 0 0 3 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285Z" />
            </svg>
          </div>
          <span class="nav-link-text ms-1">Warranty</span>
        </a>
        <div class="collapse {{ in_array(Route::currentRouteName(), ['warranty.check','warranty.list','warranty.edit','warranty.log']) ? 'show' : '' }}" id="warranty-menu">
          <ul class="nav nav-sm flex-column">
            {{-- <li class="nav-item">
              <a class="nav-link {{ Route::currentRouteName() == 'warranty.check' ? 'active' : '' }}" href="{{ route('warranty.check') }}">
                <span class="sidenav-mini-icon text-xs"> C </span>
                <span class="sidenav-normal">Check</span>
              </a>
            </li> --}}
            @can('warranty view list')
            <li class="nav-item">
              <a class="nav-link {{ in_array(Route::currentRouteName(), ['warranty.list','warranty.edit']) ? 'active' : '' }}" href="{{ route('warranty.list') }}">
                <span class="sidenav-mini-icon text-xs"> L </span>
