...
class SoStatusController extends Controller
{
  ...
  {
    $this->middleware('permission:sostatus view', ['only' => ['index', 'show']]);
  }
  ...
  public function index(Request $request)
  {
    ...
    if ($soh_txn_code != '') {
      $q->Where('a.sales_document_type', $soh_txn_code);
    }
    if ($soh_no != '') {
      $q->Where('a.sd_document', $soh_no);
    }
    if ($po_number != '') {
      $q->Where('a.purchase_order_no', $po_number);
    }
    if ($soh_cust_code != '') {
      $q->Where('a.sold_to_party', $soh_cust_code);
    }
    if ($soh_cust_name != '') {
      $q->Where('a.name1', 'like', '%' . $soh_cust_name . '%');
    }
    if ($soh_sm_code != '') {
      $q->Where('b.ZE', $soh_sm_code);
    }
    if ($sm_name != '') {
      $q->Where('c.IDMA_ZI_NAME', 'like', '%' . $sm_name . '%',);
    }
